    ...
    (set, get) => ({
      ...
      setTabViewState: (tabId, view) => {
        ...
        const next = tabs.map((t) =>
          t.id === tabId
            ? { ...t, viewState: view, postgresView: undefined }
            : t,
        );
        set({ tabs: next });
      },
      ...
      closeTabsForConnections: (ids) => {
        ...
        const next = tabs.filter(
          (t) => t.id === HOME_TAB_ID || !idSet.has(t.connectionId),
        );
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        next.tabs = next.tabs
          .filter(
            (tab) =>
              tab.id === HOME_TAB_ID || (tab.type && validTypes.has(tab.type)),
          )
          .map((tab) => ({
            ...tab,
