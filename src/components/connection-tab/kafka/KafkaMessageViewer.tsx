...
export function KafkaMessageViewer({ connectionId, config, topic, partition }: KafkaMessageViewerProps) {
  const [mode, setMode] = useState<'beginning' | 'latest' | 'offset'>('beginning')
  ...
  const consume = useCallback(async (offset: string) => {
    setLoading(true)
    setError(null)
    try {
      ...
      if (!res.ok) {
        ...
        if (res.result.messages.length > 0) {
          setLastOffset(res.result.messages[res.result.messages.length - 1].offset)
        }
      }
    } catch (err) {
  ...
  useEffect(() => {
    const offset = mode === 'beginning' ? '0' : mode === 'latest' ? '-1' : customOffset
    void consume(offset)
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <Button variant="outline" size="sm" onClick={() => void consume(mode === 'offset' ? customOffset : '0')} disabled={loading}>
          {loading ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Fetch'}
        </Button>
      </div>

      {error ? (
        <div className="rounded-md border border-destructive/40 p-2 text-xs text-destructive">{error}</div>
      ) : null}
      ...
      {result ? (
        <>
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            <span>{result.messages.length} messages {offsetLabel}</span>
            {result.hasMore ? (
              ...
              >
                ...
                >
                  ...
                  <span className="min-w-0 flex-1 truncate font-mono">
                    {msg.key !== null ? <span className="text-foreground">{msg.key}</span> : null}
                    {msg.key !== null && msg.value !== null ? <span className="text-muted-foreground"> → </span> : null}
                    {msg.value !== null ? (
                      <span className="text-muted-foreground">{truncateValue(msg.value)}</span>
                    ) : null}
                  </span>
                </button>
                {expandedIdx === idx ? (
                  <div className="border-t px-3 py-2 space-y-2">
                    ...
                    {msg.key !== null ? (
                      <>
                        <div className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Key</div>
                    ...
                    {msg.value !== null ? (
                      <>
                        <div className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Value</div>
                    ...
                    {Object.keys(msg.headers).length > 0 ? (
                      <>
                        <div className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Headers</div>
