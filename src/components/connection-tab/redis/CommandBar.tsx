...
export function CommandBar({ connectionId, config, onAfterCommand, currentKey }: CommandBarProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'ArrowUp') {
      ...
      const next = inputHistoryIndex === null ? 0 : Math.min(inputHistoryIndex + 1, inputHistory.length - 1)
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex h-9 shrink-0 items-center gap-2 border-b border-border bg-background px-3 text-xs">
        ...
        <span className="font-mono text-[10px] text-muted-foreground">redis-cli style</span>
        {currentKey && (
      ...
      <div className="relative flex h-9 shrink-0 items-center gap-2 border-b border-border px-3">
        ...
        <span className="font-mono text-[11px] text-muted-foreground">redis&gt;</span>
        <input
          ref={inputRef}
          value={input}
        ...
        <div className="space-y-1.5 p-2 font-mono text-[11px]">
          {history.length === 0 ? (
            <div className="rounded-md border border-dashed border-border p-3 text-center text-[10px] text-muted-foreground">
              Run a raw command. ↑/↓ navigates history. Strings may be single- or double-quoted.
            </div>
          ) : (
            history.map((entry) => (
...
function formatReply(reply: RedisCommandReply, indent = 0): string {
  ...
  if (typeof reply === 'string') {
    if (reply.length > 256) return `"${reply.slice(0, 256)}… (${reply.length} chars)"`
    return `"${reply}"`
  }
  ...
  if (Array.isArray(reply)) {
    ...
    return reply
      .map((item, i) => {
        ...
        return `${pad}${i + 1}) ${formatReply(item, indent + 1).split('\n').join(`\n${pad}`)}`
      })
      .join('\n')
  }
