...
export function parseCombo(combo: string): NormalizedCombo {
  ...
  const result: NormalizedCombo = { key: '', mod: false, shift: false, alt: false }
...
export function matches(combo: NormalizedCombo, event: KeyboardEvent): boolean {
  ...
  if (combo.key === '?') {
    ...
    if ((eventKey === '/' || eventCode === 'Slash') && event.shiftKey) return true
    return false
  }
...
export function formatCombo(combo: string): string {
  ...
  for (const part of parts) {
    ...
    else if (lower === 'alt' || lower === 'option') labels.push(IS_MAC ? '⌥' : 'Alt')
    else if (lower === 'ctrl' || lower === 'control') labels.push('Ctrl')
    else if (lower === 'meta' || lower === 'cmd' || lower === 'command') labels.push('⌘')
