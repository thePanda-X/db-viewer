...
const QUEUES: QueueDef[] = [
  { name: 'orders.new', exchange: 'orders', routingKey: 'order.created', durable: true },
  { name: 'orders.cancelled', exchange: 'orders', routingKey: 'order.cancelled', durable: true },
  { name: 'orders.shipped', exchange: 'orders', routingKey: 'order.shipped', durable: true },
  { name: 'notifications.email', exchange: 'notifications', routingKey: '', durable: true },
  { name: 'notifications.sms', exchange: 'notifications', routingKey: '', durable: true },
  { name: 'tasks.urgent', exchange: 'tasks', routingKey: 'urgent', durable: true },
  { name: 'tasks.default', exchange: 'tasks', routingKey: 'default', durable: true },
]

const EVENTS = ['order.created', 'order.cancelled', 'order.shipped']
const CUSTOMERS = ['Alice', 'Bob', 'Charlie', 'Diana', 'Eve']
const PRODUCTS = ['Widget Pro', 'Basic Gadget', 'Ultra Device', 'Classic Tool', 'Eco Kit']
...
function generateNotification(rand: () => number) {
  ...
  return {
    ...
    body: {
      ...
      timestamp: new Date(Date.now() - rand() * 3 * 24 * 60 * 60 * 1000).toISOString(),
    },
    contentType: 'application/json',
  }
}
...
function generateTask(rand: () => number) {
  ...
  return {
    ...
    body: {
      ...
      createdAt: new Date(Date.now() - rand() * 14 * 24 * 60 * 60 * 1000).toISOString(),
    },
    contentType: 'application/json',
  }
}
...
async function main() {
  ...
  for (const q of QUEUES) {
    ...
    console.log(`  Asserted queue "${q.name}" bound to "${q.exchange}" with key "${q.routingKey}"`)
  }
