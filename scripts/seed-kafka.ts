...
const client = new Kafka({
  brokers: [`${HOST}:${PORT}`],
  ...(USER && PASS ? { sasl: { mechanism: 'plain', username: USER, password: PASS } } : {}),
  retry: { retries: 2 },
})
...
const TOPICS: Array<{
...
}> = [
  {
    ...
    numPartitions: 3,
    messages: (rand) => {
      const customers = ['Alice Smith', 'Bob Jones', 'Charlie Brown', 'Diana Lee', 'Eve Taylor']
      const products = [
        'Premium Widget', 'Basic Gadget', 'Pro Device', 'Classic Tool',
        'Eco Kit', 'Ultra Speaker', 'Modern Lamp', 'Slim Keyboard',
      ]
      ...
      for (let i = 0; i < 100; i++) {
        ...
        messages.push({
          ...
          value: JSON.stringify({
            ...
            region: pick(['US-East', 'US-West', 'EU-West', 'EU-East', 'APAC'], rand),
            createdAt: new Date(Date.now() - rand() * 30 * 24 * 60 * 60 * 1000).toISOString(),
          }),
        })
      }
      return messages
  ...
  },
  {
    ...
    numPartitions: 2,
    messages: (rand) => {
      const pages = ['/home', '/products', '/cart', '/checkout', '/account', '/search', '/help']
      const referrers = ['google.com', 'facebook.com', 'twitter.com', 'direct', 'email']
  ...
  },
  {
    ...
    numPartitions: 3,
    messages: (rand) => {
      const eventTypes = ['signup', 'login', 'logout', 'password_reset', 'profile_update', 'delete_account']
      const messages: Array<{ key: string; value: string }> = []
      for (let i = 0; i < 150; i++) {
        ...
        messages.push({
          ...
          value: JSON.stringify({
            ...
            success: rand() > 0.1,
            timestamp: new Date(Date.now() - rand() * 14 * 24 * 60 * 60 * 1000).toISOString(),
          }),
        })
      }
      return messages
...
async function main() {
  ...
  try {
    ...
    if (CLEAR) {
      const toDelete = TOPICS.map((t) => t.name).filter((n) => existingTopics.includes(n))
    ...
    for (const topic of TOPICS) {
      await admin.createTopics({
        topics: [{ topic: topic.name, numPartitions: topic.numPartitions, replicationFactor: 1 }],
        waitForLeaders: true,
      })
      console.log(`  Created topic "${topic.name}" with ${topic.numPartitions} partition(s)`)
    }
  } finally {
