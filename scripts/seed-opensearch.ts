    ...
    docs: (rand) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        docs.push({
          ...
          tags: [pick(tags, rand), pick(tags, rand)].filter(
            (v, i, a) => a.indexOf(v) === i,
          ),
          inStock: rand() > 0.2,
          createdAt: new Date(
            Date.now() - rand() * 365 * 24 * 60 * 60 * 1000,
          ).toISOString(),
        });
      }
      return docs;
      ...
      for (let i = 0; i < count; i++) {
        ...
        docs.push({
          ...
          registeredAt: new Date(
            Date.now() - rand() * 3 * 365 * 24 * 60 * 60 * 1000,
          ).toISOString(),
          active: rand() > 0.15,
        });
      }
...
async function main() {
  ...
  for (const idx of INDEXES) {
    await client.indices.create({
      index: idx.name,
      body: { mappings: idx.mapping },
    });
    const docs = idx.docs(rand);
    const body = docs.flatMap((doc) => [
      { index: { _index: idx.name, _id: id(rand) } },
      doc,
    ]);
